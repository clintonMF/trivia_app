...
def create_app(test_config=None):
  ...
  def delete_book(question_id):
    ...
    # curl -X DELETE http://127.0.0.1:5000/questions/40
  ...
  def post_question():
    ...
    try:
      if search_term:
        questions = Question.query.filter(Question.question.ilike('%{}%'.format(search_term))).all()  #the questions for the search term
        formatted_questions = paginate(request,questions) #the search term questions formatted
      ...
      return abort(422)
    #  curl -H "Content-Type: application/json" -d '{"question":"testing","answer":"test","difficulty":3,"category":4}' -X POST http://127.0.0.1:5000/questions
# curl -H "Content-Type: application/json" -d '{"search_term": "who"}' -X POST http://127.0.0.1:5000/questions
